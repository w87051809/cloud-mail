...
const dbInit = {
	...
	async v3_1DB(c) {
		try {
			await c.env.db.batch([
				c.env.db.prepare(`ALTER TABLE setting ADD COLUMN auto_delete INTEGER NOT NULL DEFAULT 1;`),
				c.env.db.prepare(`ALTER TABLE setting ADD COLUMN auto_delete_days INTEGER NOT NULL DEFAULT 30;`)
			]);
		} catch (e) {
	...
	async v1_6DB(c) {
		...
		const noticeContent = '优秀的人，值得被好消息温柔以待。\n' +
			'<br>\n' +
			'愿你打开邮箱的这一刻，灵感、好运与安心都向你靠近。'
		...
		const ADD_COLUMN_SQL_LIST = [
			...
			`ALTER TABLE setting ADD COLUMN notice_title TEXT NOT NULL DEFAULT '欢迎回来';`,
	...
	async intDB(c) {
		...
		try {
			await c.env.db.prepare(`
			  ...
			  SELECT 1, 0, 0, 0, 'Cloud Mail', 0, 0, 1
